...
export default function SpaQuiz() {
  ...
  return (
    <>
      <Head>
        <title>Quiz — SPA — Meta Pixel Demo</title>
      </Head>

      <div id="banner">
        <span className="mode-badge mode-spa">SPA</span>
        &ensp;fbevents: <strong>{fbeventsLabel}</strong>
        &ensp;&mdash;&ensp;pixel: <strong>{pixelId}</strong>
        &ensp;<Link href="/" className="banner-link">← Reconfigure</Link>
      </div>
      ...
      <div id="quiz-wrap">
        ...
        <form id="quiz-form" action="#" method="post" autoComplete="off" onSubmit={e => e.preventDefault()}>
          ...
          {/* Step 0: Radio (visible input)
              Input click → no /tr. Label-text click → /tr fires. */}
          {!done && step === 0 && (
            <div className="step">
          ...
          {/* Step 1: Big button (hidden radio, label = button) + show-more
              InferredEvents walks up DOM from click target; fires on the wrapper
              div because its class contains "button". Same mechanism as BH's
              div.big_button wrapper. */}
          {!done && step === 1 && (
            <div className="step">
          ...
          {/* Step 2: Select with auto-advance
              Select change is not a button click → no /tr. */}
          {!done && step === 2 && (
            <div className="step">
          ...
          {/* Step 3: Select + explicit Next button
              Select change: no /tr. Button click inside form: /tr fires. */}
          {!done && step === 3 && (
            <div className="step">
          ...
          {/* Step 4: Checkboxes + conditional reveal + Next button
              Checkbox click: no /tr. Button click inside form: /tr fires. */}
          {!done && step === 4 && (
            <div className="step">
          ...
          {done && (
            <div id="completion">
              ...
              <Link href="/" className="next-btn completion-btn">← Back to configuration</Link>
            </div>
          )}
